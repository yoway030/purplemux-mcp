...
**Claude Code / Codex**에서 로컬 [purplemux](https://github.com/subicura/purplemux)(subicura의 tmux + LLM 워크스페이스 매니저)를 제어하는 MCP 서버다. 워크스페이스·탭·터미널·(Electron)브라우저·**서브에이전트 오케스트레이션**을 **23개 툴**로 다룬다.
...
- **다리 역할.** purplemux는 이 구독형 CLI들을 각각 tmux 페인(탭)으로 띄워 주고, 그걸 제어할 로컬 HTTP API를 갖고 있다. **이 MCP가 그 API를 열어주는 다리**다. 오케스트레이터(예: 지금 이 Claude Code)가 →
  1. `pmux_agent_start`로 claude/codex CLI를 탭에 런치하고 (훅 주입 + 부트 검증까지 자동)
  2. `pmux_agent_wait_ready`로 부트스트랩 echo 완료 증거를 확인하고
  3. `pmux_agent_turn`으로 턴 단위 작업을 던져 응답을 무손실 회수하고
...
> "프로세스가 떴고 모델이 실제로 응답했다"는 증거를 확보한 뒤, 응답은 파일 프로토콜(req 신원 +
...
npm run smoke     # handshake + 23툴 + 라이브 list_workspaces
